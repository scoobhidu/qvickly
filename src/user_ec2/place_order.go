...
//	if err != nil {
//		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid customer address"})
//		return
//	}
//
...
//	orderID := uuid.New()
//	err = createOrder(tx, orderID, req.CustomerID, vendor.ID, totalAmount)
//	if err != nil {
...
//	if err != nil {
//		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create vendor order items"})
//		return
//	}
//
...
//	if err != nil {
//		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create vendor assignment"})
//		return
//	}
//
...
//}
//
//func getCustomerAddress(tx *sql.Tx, customerID uuid.UUID, addressID int64) (*CustomerAddress, error) {
...
//
//	err := tx.QueryRow(query, customerID, addressID).Scan(&addr.ID, &addr.Latitude, &addr.Longitude)
//	return &addr, err
//}
//
//func findBestVendor(tx *sql.Tx, items []OrderItem, customerLat, customerLon float64) (*Vendor, float64, error) {
//	// Get all active and live vendors
//	vendorQuery := `
//		SELECT vendor_id, business_name, latitude, longitude, is_active, is_live
//		FROM quickkart.profile.vendors
//		WHERE is_active = true AND is_live = true`
//
//	rows, err := tx.Query(vendorQuery)
//	if err != nil {
//		return nil, 0, err
//	}
//	defer rows.Close()
...
//}
//
//func checkVendorInventory(tx *sql.Tx, vendorID uuid.UUID, items []OrderItem) (bool, float64, error) {
...
//
//	rows, err := tx.Query(query, vendorID, pq.Array(itemIDs))
//	if err != nil {
//		return false, 0, err
//	}
//	defer rows.Close()
...
//}
//
//func findClosestDeliveryBoy(tx *sql.Tx, vendorLat, vendorLon float64) (*DeliveryBoy, error) {
//	query := `
...
//}
//
//func createOrder(tx *sql.Tx, orderID, customerID, vendorID uuid.UUID, amount float64) error {
//	query := `
//		INSERT INTO quickkart.customer.orders (
//			order_id, customer_id, vendor_id, order_time, amount, status
//		) VALUES ($1, $2, $3, $4, $5, $6)`
//
//	_, err := tx.Exec(query, orderID, customerID, vendorID, time.Now(), amount, "placed")
//	return err
//}
//
//func createOrderItems(tx *sql.Tx, orderID uuid.UUID, items []OrderItem) error {
//	query := `
...
//}
//
//func createVendorOrderItems(tx *sql.Tx, orderID, vendorID uuid.UUID, items []OrderItem) error {
//	query := `
//		INSERT INTO quickkart.vendor.order_items (order_id, item_id, qty)
//		VALUES ($1, $2, $3)`
//
//	for _, item := range items {
//		_, err := tx.Exec(query, orderID, item.ItemID, item.Quantity)
...
//}
//
//func createVendorPickupAssignment(tx *sql.Tx, assignmentID, vendorID, orderID uuid.UUID) error {
//	query := `
//		INSERT INTO quickkart.vendor.order_pickup_assignments (
//			vendor_assignment_id, vendor_id, order_id, picked_up, created_at
//		) VALUES ($1, $2, $3, $4, $5)`
//
//	_, err := tx.Exec(query, assignmentID, vendorID, orderID, false, time.Now())
//	return err
//}
//
//func createDeliveryTracking(tx *sql.Tx, deliveryID, orderID, deliveryBoyID, vendorAssignmentID uuid.UUID) error {
//	query := `
//		INSERT INTO quickkart.delivery.order_tracker (
//			delivery_id, order_id, delivery_boy_id, vendor_assignment_id
//		) VALUES ($1, $2, $3, $4)`
//
//	_, err := tx.Exec(query, deliveryID, orderID, deliveryBoyID, vendorAssignmentID)
//	return err
//}
//
//func updateInventory(tx *sql.Tx, vendorID uuid.UUID, items []OrderItem) error {
//	query := `
...
//
//	for _, item := range items {
//		_, err := tx.Exec(query, item.Quantity, time.Now(), vendorID, item.ItemID)
//		if err != nil {
//			return err
//		}
//	}
